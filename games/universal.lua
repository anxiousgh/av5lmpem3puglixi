...
--    Player   -- working movement (walkspeed/jump/cframe/fly/noclip/inf-jump) + Fling
--    Visuals  -- ESP (box / name / distance / health, working)
--  Combat (Aimbot/Triggerbot/Target/Orbit) lives in games/combat.lua now and is
--  loaded per-game via ctx.load("games/combat.lua")(ctx) -- not by universal.
--
...
-- ============================================================
--  Fling  (velocity fling). Orbit moved to games/combat.lua (it needs the locked
--  target). "Through target" reads getgenv().WH.lockTarget (nil if combat unloaded).
-- ============================================================
local FlingSub = PlayerPage:SubPage({ Name = "Fling" })
do
    local function selectedHRP()
        -- the locked target is owned by games/combat.lua (loaded only for combat
        -- games) and published here; nil when combat isn't loaded.
        local g = getgenv and getgenv()
        local plr = g and g.WH and g.WH.lockTarget
        local ch = plr and plr.Parent and plr.Character
...
local function disableAll()
    ...
    -- visuals: flag off (their connections are disconnected below). Combat lives in
    -- games/combat.lua now and tears itself down via its own disableAll wrapper.
